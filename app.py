...
with st.sidebar:
    ...
    in_signup_depression = st.slider(
        "IN Signups", min_value=0.0, max_value=1.0, value=0.80, step=0.05, format="%.2f",
    ...
    m0_depression = st.slider(
        "M0 Depression (all channels)", min_value=0.0, max_value=1.0, value=0.90,
        step=0.05, format="%.2f", key="m0_dep"
    )
    m1_plus_depression = st.slider(
        "M1+ Depression (all channels)", min_value=0.0, max_value=1.0, value=0.90,
    ...
    active_rescue_depression = st.slider(
        "Active Rescue", min_value=0.0, max_value=1.0, value=0.90,
        step=0.05, format="%.2f", key="active_dep"
    )
    inactive_rescue_depression = st.slider(
        "Inactive Rescue", min_value=0.0, max_value=1.0, value=0.90,
...
with c1:
    st.markdown(f"""<div class="hero-card">
        <div class="hero-label">Expected Revenue at Risk (LTV)</div>
...
with c3:
    ...
    st.markdown(f"""<div class="hero-card">
        ...
        <div class="hero-sub">Expected risk &minus; extension cost</div>
    </div>""", unsafe_allow_html=True)
